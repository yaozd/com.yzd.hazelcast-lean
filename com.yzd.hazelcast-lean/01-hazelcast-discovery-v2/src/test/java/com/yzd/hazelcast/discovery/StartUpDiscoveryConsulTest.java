 ...
 * @Description:
 */
public class StartUpDiscoveryConsulTest {
    private static final String INSTANCE_NAME = "yzd";
    @Test
    public void newHazelcastByXml() {
        Config conf = new ClasspathXmlConfig("hazelcast-consul-discovery-spi-example.xml");
    }

    @Test
    public void newHazelcastWithSetConfig() throws InterruptedException {
        Config conf = new ClasspathXmlConfig("hazelcast-consul-discovery-spi-example.xml");
     ...
     */
    @Test
    public void newHazelcast() {
        Config config = new Config();
        addNetworkConfig(config);
    }

    private void addNetworkConfig(Config config) {
        NetworkConfig networkConfig = new NetworkConfig();
        networkConfig.addOutboundPort(5071);
